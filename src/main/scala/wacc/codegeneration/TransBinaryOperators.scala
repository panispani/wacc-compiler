  ...
  def transBinaryOperator(r1: Register, binOp: BinaryOperator, r2: Register): Seq[Instruction] = {
    binOp match {
      ...
      case DivBinOp  => Seq(
        MOV(R0, r1),
        MOV(R1, r2),
        BL(StaticCode.getStaticFunction(StandardLibrary.checkDivideByZero)),
      ...
      case PlusBinOp  => Seq(ADDS(r1, r1, r2), BL(StaticCode.getStaticFunction(StandardLibrary.throwOverflowError), VS))
      case MinusBinOp  => Seq(SUBS(r1, r1, r2), BL(StaticCode.getStaticFunction(StandardLibrary.throwOverflowError), VS))
